...
import { setExperienceOrder, getExperienceOrder } from '@/lib/userPreferences';
...
export async function GET() {
  try {
    ...
    console.error('Error getting experience order:', error);
    return NextResponse.json(
      { error: 'Failed to get experience order' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const success = await setExperienceOrder(user.id, orderMap);
